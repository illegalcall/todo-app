...
export default function TodoItem({ todo, onToggle, onDelete, onRename, onCancelRename }: TodoItemProps) {
  ...
  function finishEdit(save: boolean, restoreFocus = false) {
    ...
    if (save && draft.trim() && !onRename(todo.id, draft.trim())) return;
    if (!save || !draft.trim()) onCancelRename(todo.id);

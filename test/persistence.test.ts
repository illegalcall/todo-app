...
test("legacy text records retain their due date and completed state", () => {
  const original = { id: "legacy", text: "Earlier task", completed: true, dueDate: "2026-09-16" };
  assert.deepEqual(parseTodos(JSON.stringify([original])), [{ ...original, title: original.text }]);
});
